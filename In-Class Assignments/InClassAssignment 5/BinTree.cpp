...
void BinTree<T>::insert(Node<T>* parent, Node<T>* node) {
	if (*parent < *node) { // checks to see if node should be inserted in right subtree
		...
		*(parent->data) + *(node->data);
		...
		if (!parent->left) { // if paren'ts left child is null, then inserts the node there
			parent->left = node;
			size++;
		}
		else
...
Node<T>* BinTree<T>::remove(Node<T>* parent, T* value)
{
	...
	if (*parent->data > *value) // value is in left subtree
...
template<typename T>
void BinTree<T>::printBreadthFirst()
{
	std::queue<Node<T>*> q;
	if (root) {
		q.push(root);
...
template<typename T>
int BinTree<T>::getHeight(Node<T>* node)
{
	if (node)
